...
export default function GuiaComprador() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Descarga del archivo PDF (debe estar en /public/guia-inmobiliaria.pdf)
    const link = document.createElement('a');
    link.href = '/guia-inmobiliaria.pdf';
    link.download = 'Guia-Inmobiliaria-Miami.pdf';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    // 2. Preparar mensaje para WhatsApp
    const message = `Hola Yadira, mi nombre es ${formData.name}. Acabo de descargar la guía inmobiliaria desde tu web. Me gustaría recibir más información. Mi correo es: ${formData.email}`;
      ...
      <div className="mx-auto max-w-6xl px-6 py-20 grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[#1E293B] mb-6 leading-tight">
            Tu hoja de ruta para conquistar el mercado inmobiliario de Miami
          </h1>
          <p className="text-lg text-gray-600 mb-8">
            Evita errores comunes y toma decisiones financieras inteligentes. He destilado años de experiencia 
            en el mercado del sur de la Florida para que tú no tengas que adivinar.
          </p>
          
          <ul className="space-y-4 mb-8">
            {[
              "El paso a paso: Del pre-aprobado al cierre sin sorpresas.", 
              "Las 5 'red flags' que los inversores inteligentes revisan antes de firmar.", 
              "Radiografía del mercado: ¿Cómo saber si es el momento perfecto para comprar?"
            ].map((item, i) => (
        ...
        >
          <h3 className="text-2xl font-bold text-[#1E293B] mb-6">Obtén tu guía gratuita</h3>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex items-start gap-2 pt-2">
              ...
              <label className="text-[11px] text-gray-600 leading-relaxed text-justify">
                Acepto los términos y condiciones y la Política de privacidad proporcionados por la empresa. 
                Acepto ser contactado por Yadira Portuondo por llamada, correo electrónico y mensaje de texto.
            ...
            >
              {isSubmitting ? "Procesando..." : "Descarga tu guía"}
            </button>
          </form>
        </motion.div>
      </div>
